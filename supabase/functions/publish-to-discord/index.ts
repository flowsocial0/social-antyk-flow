...
Deno.serve(async (req) => {
  ...
  try {
    console.log('=== Publish to Discord Request ===');
    ...
    if (!userId) throw new Error('Musisz być zalogowany aby publikować na Discord');

    const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);

    // Get Discord webhook config
    let tokenQuery = supabase.from('discord_tokens').select('*').eq('user_id', userId);
    if (accountId) tokenQuery = tokenQuery.eq('id', accountId);
    const { data: tokenData } = await tokenQuery.limit(1).maybeSingle();

    if (!tokenData) {
      throw new Error('Discord nie jest skonfigurowany. Dodaj Webhook URL w ustawieniach.');
    }

    const { webhook_url, channel_name } = tokenData;

    // Test connection
    if (testConnection || (!bookId && !contentId && !campaignPostId && !text && !imageUrl && !videoUrl)) {
      // Test webhook by sending a GET (Discord webhooks support GET to get info)
      const testResp = await fetch(webhook_url);
      const isOk = testResp.ok;
      return new Response(JSON.stringify({
        success: isOk,
        connected: isOk,
        accountName: channel_name || 'Discord Webhook',
        platform: 'discord',
    ...
    const payload: any = { content: postText || undefined };
    ...
    console.log('Published successfully to Discord!');
    ...
    return new Response(JSON.stringify({
      success: true,
      message: 'Pomyślnie opublikowano na Discord',
      results: [{ success: true, platform: 'discord' }],
    }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });

  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : 'Nieznany błąd';
    console.error('Error in publish-to-discord:', error);
    return new Response(JSON.stringify({
      success: false, error: errorMessage,
      results: [{ success: false, platform: 'discord', error: errorMessage }],
